...
//==================================================================================================
	...
	$prgname = "会場予約注文状況一覧画面";
	$prgmemo = "　会場予約注文の状況を確認できます。";
...
<div id="container">
	...
	<div id="contents">
		<div id="main">
			<h1><u><b><font size="20">会場予約状況一覧</font></b></u></h1>
			<? echo $prgmemo; ?>
			<p>
			＜検索方法＞<br>
			①Ctrl + F<br>
			...
			<div id="formWrap">
				...
				<h2>検索条件</h2><br>
					...
					<table class="tbd" cellspacing="0" cellpadding="0" border="0" summary="ベーステーブル" id= "TBL">
						<tr>
							<th class="tbd_th_p1"><strong>注文No.</strong></th>
							<th class="tbd_th_p1"><strong>受付日<br>会場<br>担当者</strong></th>
							<th class="tbd_th_p1" colspan="3"><strong>購入情報</strong></th>
							<th class="tbd_th_p1"><strong>状況</strong></th>
						</tr>
					<?
					// ================================================
					// ■　□　■　□　個別表示　■　□　■　□
					...
					$query  = "
						SELECT
							 A.idxnum  ,A.g_buydt  ,A.g_staff, A.g_locale 
							,A.name    ,A.modelnum      ,A.cash    ,A.buynum
							,A.option1 ,A.option2 ,A.option3 ,A.option4 ,A.option5 ,A.option6 ,A.option7 ,A.option8 ,A.option9 ,A.option10
							,A.slipnumber	,A.phonenum1, A.b_way, A.p_way
							,CASE 
								WHEN A.delflg = '1' THEN 'キャンセル済'
								WHEN C.slipnumber IS NOT NULL OR A.g_buydt<'2022-09-01' THEN '発送済'
						 ...
						 WHERE A.reservflg > 0 
						   AND A.g_buydt like '$ym%'
						 ORDER BY A.g_buydt, A.venueid, A.venueno, A.idxnum DESC
					";
					...
					?>
						...
						<? } ?>
							<? if ($p_compcd == "S") { ?>
								<td class="tbd_td_p1_c" rowspan="3"><? echo $row['idxnum'] ?></td>
								<td class="tbd_td_p1_l" rowspan="3">
							<? } else { ?>
								<td class="tbd_td_p1_c" rowspan="2"><? echo $row['idxnum'] ?></td>
								<td class="tbd_td_p1_l" rowspan="2">
							<? } ?>
								<? echo date('m/d', strtotime($row['g_buydt'])) ?><br>
								<? echo $row['g_locale'] ?><br>
								<? echo $row['g_staff'] ?><br>
								<? echo "【".$row['p_way']."】";?><br>
								<? if ($row['slipnumber'] > 0) { ?>
									<?
							...
							<? if ($p_compcd == "S") { ?>
								<td class="tbd_td_p2_c" rowspan="3"><font color="<? echo $arr_color[$row['status']] ?>"><? echo $row['status']; ?></font></td>
							<? } else { ?>
								<td class="tbd_td_p2_c" rowspan="2"><font color="<? echo $arr_color[$row['status']] ?>"><? echo $row['status']; ?></font></td>
							<? } ?>
						</tr>
						<!-- ２段目 -->
						<?
						...
						<? } ?>
							<td class="tbd_td_p2_l">
								<? echo $row['modelnum'] ?>
								<? if ($row['b_way'] == "下取") {echo "【下取】";} ?>
								<br>
								<? echo implode(',',$arr_option) ?>
							</td>
							<td class="tbd_td_p1_r"><? echo $row['buynum'] ?>台</td>
							<td class="tbd_td_p2_r"><? echo number_format($row['cash']*100) ?>円</td>
						</tr>
					<?
						//日付を退避
						$buydt = $row['buydt'];
